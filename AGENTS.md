# ai_agent_assistant — Codex Guide

> Read this at session start. It tells Codex what this project is, how to run it, and how to log progress into the user's Obsidian vault.
...
**Fallback chain:** Ollama → Groq → Gemini → OpenAI → Codex.
...
- API keys (OpenAI, Groq, Gemini, HuggingFace, Codex, Cloudflare) — those live in `400 Projects/Credentials/AI Assistant.md` only
...
Before wrapping a Codex session, glance at `400 Projects/Coding/ai_agent_assistant/progress.md` and confirm a new entry exists if the session did anything meaningful.
